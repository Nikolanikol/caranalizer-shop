...
/**
 * Порог «страница переживёт продажу» — что пускаем в индекс и в карту сайта.
 *
 * Карточка, за которой стоит один экземпляр, живёт до первой продажи: товар ушёл —
 * страница умерла. Таких 12 821 из 18 655, и класть их в карту сайта значит заранее
 * наплодить 404 там, где уже 307 755 адресов висят непроиндексированными. Страница
 * у них есть (иначе товар не положить в корзину), но `noindex` и вне карты сайта.
 *
 * **Это решение про SEO. Трогать его ради скорости сборки нельзя** — для сборки есть
 * второй порог, ниже.
 */
export const INDEXABLE_MIN_OFFERS = 2;

/**
 * Порог «собираем заранее» — сколько карточек кладём в образ.
 *
 * Разведён с индексным 02.09.2026, и вот почему. Обе цифры брались из одной функции,
 * поэтому поднять порог сборки было нельзя: вместе с ней похудела бы карта сайта.
 * А платить за пререндер приходилось трижды — по логу деплоя 02.09:
 *
 * ```
 * Generating static pages (8162)   198 с
 * COPY --chown … /.next/standalone  55 с
 * exporting layers + unpacking     141 с
 * ```
 *
 * Это 394 секунды из 530, то есть три четверти сборки образа. И покупалось этим
 * немного: у карточки `revalidate: 1d`, значит через сутки после выкатки она всё равно
 * пересчитывается по запросу — пререндер экономит холодный первый заход только
 * в первые сутки. Замерено, сколько стоит этот холодный заход: **49–127 мс против
 * 3–16 мс у собранной заранее**, а со второго захода они неотличимы.
 *
 * При пяти экземплярах собирается 1 142 карточки вместо 5 834. `dynamicParams = true`,
 * так что остальные рендерятся по первому запросу и кэшируются — из индекса и из карты
 * сайта не пропадает ни одна.
 *
 * Захочется дожать — дорога до нуля открыта и стоит одного числа; смотреть при этом
 * на среднее время ответа в Search Console → Статистика сканирования.
 */
export const PRERENDER_MIN_OFFERS = 5;

async function partsWithMinOffers(min: number): Promise<AutoPart[]> {
  const db = createServerClient();
  const rows = await selectAll<ProductRow>((from, to) =>
    db.from('partsfit_products').select(PRODUCT_FIELDS).gte('offers_count', min).order('id').range(from, to),
  );
  return rows.map(toPart);
}

/** Детали, которым положена страница в индексе и в карте сайта. */
export function getIndexableParts(): Promise<AutoPart[]> {
  return partsWithMinOffers(INDEXABLE_MIN_OFFERS);
}

/** Детали, которые собираем заранее. Подмножество индексируемых, а не другой набор. */
export function getPrerenderParts(): Promise<AutoPart[]> {
  return partsWithMinOffers(PRERENDER_MIN_OFFERS);
}
